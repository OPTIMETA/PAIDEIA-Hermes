...
```bash
python3 "${PAIDEIA_PLUGIN_ROOT}/pd_vision_ocr.py" --engine=ollama <input.pdf> <output.md>
```

`${PAIDEIA_PLUGIN_ROOT}/pd_vision_ocr.py` is the single source of truth. It:
...
```bash
python3 "${PAIDEIA_PLUGIN_ROOT}/pd_vision_ocr.py" --engine=tesseract <input.pdf> <output.md>
```
